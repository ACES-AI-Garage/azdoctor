...
function detectWebhookType(url: string): "teams" | "slack" | "generic" {
  if (url.includes("webhook.office.com") || url.includes("microsoft.com")) return "teams";
  if (url.includes("hooks.slack.com")) return "slack";
  return "generic";
}

function formatTeamsMessage(title: string, message: string, severity: string): object {
  return {
    "@type": "MessageCard",
...
function formatSlackMessage(title: string, message: string, severity: string): object {
...
function formatGenericMessage(title: string, message: string, severity: string): object {
  return {
    title,
    severity,
    message,
...
async function sendWebhook(url: string, payload: object): Promise<{ success: boolean; statusCode: number; error?: string }> {
  try {
...
function summarizeToolOutput(message: string): string {
  try {
    ...
    if (parsed.riskScore !== undefined) parts.push(`**Risk Score:** ${parsed.riskScore}/100`);
    if (parsed.diagnosticCoverage) parts.push(`**Coverage:** ${parsed.diagnosticCoverage}`);

    if (parsed.diagnosticInsights?.length > 0) {
      parts.push(`**Patterns Detected:** ${parsed.diagnosticInsights.map((i: any) => i.pattern).join(", ")}`);
    }

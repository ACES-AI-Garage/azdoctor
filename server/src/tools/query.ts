...
interface QueryTemplate {
  ...
  table: string;
  label: string;
  buildQuery: (match: RegExpMatchArray, timeframeHours: number, maxRows: number) => string;
  suggestions: string[];
}
...
const QUERY_TEMPLATES: QueryTemplate[] = [
  {
    ...
    buildQuery: (_match, hours, maxRows) =>
      `AppRequests
...
function isRawKql(description: string): boolean {
...
function matchTemplate(
  ...
  maxRows: number,
): { kql: string; matchedPattern: string; suggestions: string[] } | null {
                ...
                text: JSON.stringify(
                  {
                    error: {
                      ...
                      message:
                        "Either 'workspaceId' or 'resourceGroup' must be provided so a Log Analytics workspace can be resolved.",
                    },
                  },
                  null,
                  2,

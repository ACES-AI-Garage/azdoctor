...
interface ScopeSummary {
  ...
  healthSummary: { healthy: number; degraded: number; unavailable: number };
  recentChanges: number;
}

interface Difference {
  category: "resource_types" | "health" | "change_velocity";
  detail: string;
...
interface ComparisonResult {
  ...
  parity: "matched" | "partial" | "divergent";
  timestamp: string;
  errors?: AzureError[];
}
...
function isSubscriptionId(scope: string): boolean {
  return GUID_PATTERN.test(scope);
}

async function gatherScopeSummary(
  scope: string,
  subscription: string,
  mode: "health" | "resources" | "full",
  errors: AzureError[]
  ...
  const promises: [
    Promise<Awaited<ReturnType<typeof queryResourceGraph>>>,
    Promise<Awaited<ReturnType<typeof batchResourceHealth>>>,
    Promise<Awaited<ReturnType<typeof getActivityLogs>>>,
  ] = [
    // Resource inventory
    mode === "health"
  ...
  const [resourceResult, healthResult, activityResult] =
  ...
  server.tool(
    ...
    async ({ scopeA, scopeB, subscription: subParam, mode }) => {
      ...
      if (mode !== "resources") {
        ...
        const unhealthyA =
          summaryA.healthSummary.degraded + summaryA.healthSummary.unavailable;
        const unhealthyB =
          summaryB.healthSummary.degraded + summaryB.healthSummary.unavailable;

        if (
          (unhealthyA > 0 && unhealthyB === 0) ||
      ...
      let parity: "matched" | "partial" | "divergent";
      if (hasCritical) {
        parity = "divergent";
      } else if (hasWarning) {
        parity = "partial";

...
export function registerInvestigate(server: McpServer): void {
  server.tool(
    ...
    async ({
      ...
      let logAnalyticsInsights: Array<{ workspace: string; errorCount: number; topOperations: string[] }> = [];
      if (resolvedResourceGroup) {
...
| take 5`;
              ...
              const topOperations = rows
                ?.map((row) => `${String(row[1] ?? "Unknown")} (${row[2]} errors)`)
                .slice(0, 5) ?? [];

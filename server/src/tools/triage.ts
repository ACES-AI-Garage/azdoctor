...
import type { DiagnosticEvent, TrendResult, DiagnosticInsight } from "../utils/correlator.js";
...
const TRIAGE_ALERT_TEMPLATES: Record<string, SimpleAlertRec[]> = {
  "microsoft.web/sites": [
...
function buildJournalMarkdown(report: {
  ...
  topology: string;
  alertRecommendations: SimpleAlertRec[];
}): string {
  const insightsBullets =
    report.diagnosticInsights.length > 0
    ...
    report.baseline.metrics.length > 0
      ? "| Metric | Current | Mean | StdDev | Z-Score | Status |\n|--------|---------|------|--------|---------|--------|\n" +
        report.baseline.metrics
          .map(
            (m) =>
              `| ${m.metric} | ${m.current} | ${m.mean} | ${m.stdDev} | ${m.zScore} | ${m.status} |`
          )
          .join("\n")
...
export function registerTriage(server: McpServer): void {
  server.tool(
    ...
    async ({
      ...
      const [healthResult, activityResult, metricsResult, workspacesResult] =
        await Promise.all([
      ...
      if (activityResult.error) {
        ...
        for (const event of activityResult.events) {
          ...
          const timestamp =
            event.eventTimestamp?.toISOString() ?? new Date().toISOString();
          allEvents.push({
            time: timestamp,
            event: `${opName} (${status})`,
            source: "ActivityLog",
      ...
      const metricDataByName: Map<
        string,
        Array<{ timestamp: string; average?: number; maximum?: number }>
      > = new Map();
        ...
        for (const metric of metricsResult.data.metrics) {
          for (const ts of metric.timeseries) {
            ...
            const dataPoints = ts.data
              .filter(
                (dp) => dp.average !== undefined || dp.maximum !== undefined
              )
              .map((dp) => ({
                timestamp:
      ...
      const depNodes: TopologyNode[] = dependentResources.map((d) => ({
        name: d.name,
        type: d.type,
      ...
      const alertRecommendations: SimpleAlertRec[] = generateAlerts

...
export function registerHealthcheck(server: McpServer): void {
  server.tool(
    ...
    async ({ subscription: subParam, resourceGroup, severity }) => {
      ...
      const [healthResult, activityResult, ...rgCheckResults] =
        await Promise.all([
          // Health check
      ...
      if (activityResult.error) {
        ...
        for (const event of activityResult.events) {
          ...
          if (
            ...
            failedDeployments++;
            const resourceName =
              event.resourceId?.split("/").pop() ?? "unknown";
            findings.push({
              severity: "warning",
              resource: resourceName,
      ...
      if (unattachedDisksResult.error) {
        ...
        for (const disk of unattachedDisksResult.resources) {
          ...
          findings.push({
            ...
            recommendation:
              "Delete the disk if no longer needed, or reattach it to a VM to avoid wasted cost.",
          });
        }
      }
      ...
      if (unassociatedIPsResult.error) {
        ...
        for (const ip of unassociatedIPsResult.resources) {
          ...
          findings.push({
            ...
            recommendation:
              "Review whether this public IP is still needed. Unassociated public IPs may pose a security risk and incur cost.",
          });
        }
      }
      ...
      if (classicResourcesResult.error) {
        ...
        for (const res of classicResourcesResult.resources) {
          ...
          findings.push({
            ...
            recommendation:
              "Migrate to Azure Resource Manager (ARM). Classic resources will be retired. See https://aka.ms/classicresourcemigration.",
          });
        }
      }
      ...
      if (criticalResourcesResult.error) {
        ...
        for (const res of criticalResourcesResult.resources) {
          findings.push({
            ...
            recommendation:
              "Consider adding a CanNotDelete or ReadOnly lock to protect this critical resource from accidental deletion or modification.",
          });
        }
      }

...
const ANOMALY_SOURCES = new Set<string>(["ResourceHealth", "Metrics", "ServiceHealth"]);

/** Sources that represent changes (something was done) */
const CHANGE_SOURCES = new Set<string>(["ActivityLog"]);
...
export function clusterEvents(
  ...
  let currentCluster: DiagnosticEvent[] = [sorted[0]];
...
function buildCluster(events: DiagnosticEvent[]): EventCluster {
  const sources = [...new Set(events.map((e) => e.source))];
  const highestSeverity = events.reduce<"critical" | "warning" | "info">(
    (max, e) => {
      const rank = SEVERITY_RANK[e.severity ?? "info"] ?? 1;
      return rank > (SEVERITY_RANK[max] ?? 1) ? (e.severity as "critical" | "warning" | "info") : max;
    },
    "info"
  );
 ...
 */
export function detectChangeVelocity(
  events: DiagnosticEvent[],
  windowMinutes: number = 30,
  threshold: number = 5
): ChangeVelocityResult {
  const changes = events
    .filter((e) => CHANGE_SOURCES.has(e.source))
    .sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());

  const windowMs = windowMinutes * 60 * 1000;
  const highVelocityPeriods: ChangeVelocityResult["highVelocityPeriods"] = [];
...
function buildCausalNarrative(
  earliestAnomaly: DiagnosticEvent,
  precedingChanges: DiagnosticEvent[],
  allAnomalies: DiagnosticEvent[],
  ...
  if (precedingChanges.length > 0) {
    ...
    const gapMinutes = Math.round(
      (anomalyTime.getTime() - changeTime.getTime()) / 60000
    );
...
// ---------------------------------------------------------------------------
// Service-Specific Diagnostic Heuristics
// ---------------------------------------------------------------------------
 ...
 */
export function detectDiagnosticPatterns(
  events: DiagnosticEvent[],
  resourceType: string
): DiagnosticInsight[] {
  ...
  const findEvents = (substring: string): DiagnosticEvent[] =>
    events.filter((e) => e.event.toLowerCase().includes(substring.toLowerCase()));

  const findEventsBySeverity = (
    substring: string,
    ...severities: string[]
  ): DiagnosticEvent[] =>
    events.filter(
      (e) =>
        e.event.toLowerCase().includes(substring.toLowerCase()) &&
        severities.includes(e.severity ?? "info")
    );

  const findEventsBySource = (source: string): DiagnosticEvent[] =>
  ...
  const anyWithinMinutes = (
    groupA: DiagnosticEvent[],
    groupB: DiagnosticEvent[],
    minutes: number
  ): boolean => {
  ...
  if (normalizedType === "microsoft.web/sites") {
    ...
    const deployments = findEventsBySource("ActivityLog").filter(
      (e) =>
        e.event.toLowerCase().includes("deploy") ||
    ...
    if (
      ...
      insights.push({
        ...
        description:
          "A deployment or configuration change occurred shortly before HTTP errors or response time degradation.",
        confidence: "high",
        evidence: [
        ...
        recommendation:
          "Recent deployment likely caused errors. Consider rolling back.",
      });
    }
    ...
    if (memoryCritical.length > 0 && queueOrResp.length > 0) {
      insights.push({
        ...
        description:
          "High memory usage is causing request queuing and response time degradation.",
        confidence: "high",
        evidence: [
        ...
        recommendation:
          "Memory pressure causing request queuing. Scale up the App Service Plan or optimize memory usage.",
      });
    }
    ...
    if (cpuCritical.length > 0 && httpRespWarnCrit.length > 0) {
      insights.push({
        ...
        description:
          "High CPU usage is degrading HTTP response times.",
        confidence: "high",
        evidence: [
        ...
        recommendation:
          "CPU saturation degrading response times. Scale out or optimize CPU-intensive operations.",
      });
    }
    ...
    if (healthCheck.length > 0) {
      insights.push({
        ...
        recommendation:
          "Health check endpoint is failing. Review the health check path configuration and application startup.",
      });
    }
  }
  ...
  if (normalizedType === "microsoft.sql/servers/databases") {
    ...
    if (dtuCritical.length > 0) {
      insights.push({
        ...
        recommendation:
          "DTU capacity exhausted. Scale up the database tier or optimize queries.",
      });
    }
    ...
    if (connFailed.length > 0) {
      insights.push({
        ...
        recommendation:
          "Connection failures detected. Check connection pool settings and max connection limits.",
      });
    }
    ...
    if (deadlocks.length > 0 && dtuWarning.length > 0) {
      insights.push({
        ...
        description:
          "Deadlocks are occurring alongside elevated DTU consumption.",
        confidence: "high",
        evidence: [
        ...
        recommendation:
          "Deadlocks under load. Review transaction isolation levels and query patterns.",
      });
    }
  }
  ...
  if (normalizedType === "microsoft.compute/virtualmachines") {
    ...
    if (diskQueue.length > 0) {
      insights.push({
        ...
        recommendation:
          "Disk I/O bottleneck. Consider Premium SSD or Ultra Disk, or distribute I/O across multiple disks.",
      });
    }
    ...
    if (networkEvents.length > 0) {
      insights.push({
        ...
        recommendation:
          "Network bandwidth saturated. Consider accelerated networking or scaling to a larger VM size.",
      });
    }
  }
  ...
  if (healthUnavailable.length > 0) {
    ...
    if (!hasPrecedingChange) {
      insights.push({
        ...
        description:
          "Resource became unavailable with no preceding configuration changes.",
        confidence: "medium",
        evidence: healthUnavailable.map(
          (e) => `[ResourceHealth] ${e.event} at ${e.time}`
        ),
        recommendation:
          "Platform-level incident suspected. Check Azure Service Health.",
      });
    }
  }
    ...
    for (let i = 0; i <= sorted.length - 5 && !found; i++) {
      ...
      if (endTime - startTime <= tenMinMs) {
        ...
        insights.push({
          ...
          description:
            "Multiple configuration changes occurred in a short time window.",
          confidence: "medium",
          evidence: sorted.map(
            (e) => `[ActivityLog] ${e.event} at ${e.time}`
          ),
          recommendation:
            "Rapid configuration changes detected. This may indicate automated remediation loops or deployment issues.",
        });
      }
    }
  }

 ...
 */
export function formatRCA(input: RCAInput): string {
  const timelineRows =
    input.timeline.length > 0
      ? input.timeline
          .map(
            (e) =>
              `| ${e.time} | ${e.event} | ${e.source} |${e.resource ? ` ${e.resource}` : ""}`
          )
          .join("\n")
      : "| — | No events recorded | — |";

  const affectedList =
    input.impact.affectedResources.length > 0
      ? input.impact.affectedResources.join(", ")
      : "None identified";

  const remediationList =
    input.remediationApplied.length > 0
...
export function formatHealthSummary(
  ...
  const rows = findings
    .map(
      (f) =>
        `| ${severityIcon(f.severity)} ${f.severity.toUpperCase()} | ${f.resource} | ${f.issue} | ${f.recommendation} |`
    )
    .join("\n");

import { z } from "zod";
import { resolveSubscription, queryResourceGraph, getResourceHealth, getActivityLogs, getMetrics, batchExecute, } from "../utils/azure-client.js";
...
import { renderTopology, renderMermaidTopology, renderMermaidTimeline, } from "../utils/formatters.js";
export function registerDiagram(server) {
    server.tool("azdoctor_diagram", "Generate Mermaid diagrams from investigation data. Produces dependency topology diagrams and incident timeline diagrams that render in GitHub, VS Code, and documentation tools.", {
    ...
    }, async ({ resource, subscription: subParam, resourceGroup, diagramType, timeframeHours, }) => {
        ...
        if (diagramType === "topology" || diagramType === "both") {
            ...
            const rootHealth = rootHealthResult.statuses[0]?.properties?.availabilityState ??
                "Unknown";
            ...
            if (resolvedResourceGroup) {
                ...
                if (depQueries.length > 0) {
                    ...
                    const healthChecks = await batchExecute(Array.from(allDeps.values()).map((dep) => async () => {
                        const depHealth = await getResourceHealth(subscription, dep.id);
                        const depState = depHealth.statuses[0]?.properties?.availabilityState ??
                            "Unknown";
                        return { dep, depState };
                    }), 5);
        ...
        if (diagramType === "timeline" || diagramType === "both") {
            ...
            timelineMermaid = renderMermaidTimeline(correlation.timeline.map((e) => ({
                time: e.time,
                event: e.event,
                source: e.source,
                severity: e.severity,

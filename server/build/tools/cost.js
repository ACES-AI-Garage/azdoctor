import { z } from "zod";
import { resolveSubscription, queryResourceGraph, getMetrics, batchExecute, } from "../utils/azure-client.js";
// ─── VM downsize mapping ────────────────────────────────────────────
const VM_DOWNSIZE_MAP = {
...
// ─── Registration ───────────────────────────────────────────────────
export function registerCost(server) {
    server.tool("azdoctor_cost", "Analyze Azure resource costs and identify waste. Detects idle resources, oversized VMs, unattached storage, and recommends right-sizing based on usage metrics.", {
        subscription: z
            .string()
            .optional()
        ...
        const [unattachedDisksResult, stoppedVmsResult, appServicePlansResult, unassociatedIpsResult, loadBalancersResult, runningVmsResult,] = await Promise.all([
            // Unattached disks
            queryResourceGraph([subscription], `Resources | where ${rgFilter}type =~ 'Microsoft.Compute/disks' and properties.diskState == 'Unattached' | project id, name, type, location, resourceGroup, sku_name=tostring(sku.name), diskSizeGB=toint(properties.diskSizeGB)`),
            // Stopped but allocated VMs
            queryResourceGraph([subscription], `Resources | where ${rgFilter}type =~ 'Microsoft.Compute/virtualMachines' and properties.extended.instanceView.powerState.code == 'PowerState/stopped' | project id, name, type, location, resourceGroup, vmSize=tostring(properties.hardwareProfile.vmSize)`),
            // App Service Plans (check for empty ones)
            queryResourceGraph([subscription], `Resources | where ${rgFilter}type =~ 'Microsoft.Web/serverfarms' | project id, name, resourceGroup, sku_name=tostring(sku.name), sku_tier=tostring(sku.tier), numberOfSites=toint(properties.numberOfSites)`),
            // Unassociated Public IPs
            queryResourceGraph([subscription], `Resources | where ${rgFilter}type =~ 'Microsoft.Network/publicIPAddresses' and properties.ipConfiguration == '' | project id, name, resourceGroup, sku_name=tostring(sku.name)`),
            // Load Balancers (check for idle ones)
            queryResourceGraph([subscription], `Resources | where ${rgFilter}type =~ 'Microsoft.Network/loadBalancers' | project id, name, resourceGroup, sku_name=tostring(sku.name), backendPools=properties.backendAddressPools`),
            // Running VMs for right-sizing analysis
            queryResourceGraph([subscription], `Resources | where ${rgFilter}type =~ 'Microsoft.Compute/virtualMachines' and properties.extended.instanceView.powerState.code == 'PowerState/running' | project id, name, resourceGroup, vmSize=tostring(properties.hardwareProfile.vmSize)`),
        ]);
        ...
        if (unassociatedIpsResult.error) {
            ...
            for (const ip of unassociatedIpsResult.resources) {
                ...
                const isStandard = skuName?.toLowerCase() === "standard" ||
        ...
        if (runningVmsResult.error) {
            ...
            const metricTasks = vmsToAnalyze.map((vm) => {
                return async () => {
                    ...
                    const metricsResult = await getMetrics(resourceId, ["Percentage CPU"], lookbackDays * 24, "PT1H");

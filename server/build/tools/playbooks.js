...
function matchPlaybooks(playbooks, context) {
    ...
    for (const playbook of playbooks) {
        ...
        for (const symptom of triggers.symptoms) {
            ...
            const matched = symptoms.some((s) => s.toLowerCase().includes(symptomLower)) ||
    ...
    results.sort((a, b) => (relevanceOrder[a.relevance] ?? 2) - (relevanceOrder[b.relevance] ?? 2));
    return results;
}
/**
...
export function registerPlaybooks(server) {
    server.tool("azdoctor_playbooks", "Manage custom diagnostic playbooks. Users can create playbooks in ~/.azdoctor/playbooks/ to define custom diagnostic patterns and remediation steps. Use 'list' to see available playbooks, 'show' to view one, or 'match' to find playbooks relevant to an investigation.", {
        action: z
            .enum(["list", "show", "match", "init"])
            .describe("Action: list available playbooks, show a specific one, match against investigation findings, or init to create a sample playbook"),
        playbookName: z
            .string()
            .optional()
        ...
        investigationContext: z
            .string()
            .optional()
            .describe("JSON output from azdoctor_investigate — used by 'match' action to find relevant playbooks"),
        ...
        try {
            switch (action) {
                ...
                case "show": {
                    ...
                    const found = playbooks.find((p) => p.frontmatter.name.toLowerCase() === nameLower ||
                            ...
                            content: [
                                {
                                    ...
                                    text: JSON.stringify({
                                        ...
                                        availablePlaybooks: playbooks.map((p) => p.frontmatter.name),
                                    }),
                                },
                            ],
                            isError: true,
                ...
                case "match": {
                            ...
                            content: [
                                {
                                    ...
                                    text: JSON.stringify({
                                        matchedPlaybooks: [],
                                        totalMatched: 0,
                                        investigatedResource: String(context["resource"] ?? context["resourceId"] ?? "unknown"),
                    ...
                    const investigatedResource = String(context["resource"] ?? context["resourceId"] ?? "unknown");
                    const result = {
                ...
                case "init": {
                    ...
                    const sampleFiles = [];
                    const highMemoryPath = join(PLAYBOOKS_DIR, "example-high-memory.md");
                    ...
                    const connFailuresPath = join(PLAYBOOKS_DIR, "example-connection-failures.md");
                    if (!existsSync(connFailuresPath)) {
                        writeFileSync(connFailuresPath, SAMPLE_CONNECTION_FAILURES.trimStart(), "utf-8");

...
function tailorRecommendations(recommendations, investigationContext) {
    ...
    try {
        ...
        if (Array.isArray(context.metricTrends)) {
            const risingTrends = context.metricTrends.filter((t) => t.direction === "rising");
                ...
                for (const trend of risingTrends) {
                    ...
                    if (existing) {
                        const proactive = {
                            ...
                            threshold: existing.operator === "GreaterThan"
...
function generateBicep(resourceId, recommendations) {
    ...
    for (const rec of recommendations) {
        ...
        lines.push(`resource alert_${sanitized} 'Microsoft.Insights/metricAlerts@2018-03-01' = {`, `  name: 'azdoctor-${rec.name.replace(/'/g, "")}'`, `  location: 'global'`, `  properties: {`, `    severity: ${rec.severity}`, `    enabled: true`, `    scopes: [resourceId]`, `    evaluationFrequency: '${rec.frequency}'`, `    windowSize: '${rec.windowSize}'`, `    criteria: {`, `      'odata.type': 'Microsoft.Azure.Monitor.SingleResourceMultipleMetricCriteria'`, `      allOf: [`, `        {`, `          name: '${rec.metricName}'`, `          metricName: '${rec.metricName}'`, `          operator: '${rec.operator}'`, `          threshold: ${rec.threshold}`, `          timeAggregation: 'Average'`, `          criterionType: 'StaticThresholdCriterion'`, `        }`, `      ]`, `    }`, `    actions: actionGroupId != '' ? [{ actionGroupId: actionGroupId }] : []`, `  }`, `}`, "");
    }
    return lines.join("\n");
}
export function registerAlertRules(server) {
    server.tool("azdoctor_alert_rules", "Generate Azure Monitor alert rule recommendations based on resource type and investigation findings. Outputs deployable Bicep templates.", {
        resource: z
            .string()
        ...
        investigationContext: z
            .string()
            .optional()
            .describe("JSON output from a prior azdoctor_investigate call — used to tailor alerts to the specific issues found"),
        ...
        if (investigationContext) {
            const result = tailorRecommendations(recommendations, investigationContext);

import { z } from "zod";
import { resolveSubscription, queryResourceGraph, getResourceHealth, getActivityLogs, getMetrics, querySqlQueryStore, } from "../utils/azure-client.js";
    ...
    server.tool("azdoctor_investigate", "Investigate a specific Azure resource or incident. Performs multi-signal correlation across Resource Health, Activity Logs, Metrics, and dependent resources to identify root cause.", {
        ...
        const [healthResult, activityResult, metricsResult] = await Promise.all([
            ...
            (async () => {
                ...
                if (metricConfig) {
                    return getMetrics(resourceId, metricConfig.names, timeframeHours);
                }
        ...
        let queryStoreInsights;
        if (resourceType.toLowerCase() === "microsoft.sql/servers/databases") {
            const hasPerformanceAnomaly = allEvents.some((e) => e.source === "Metrics" &&
            ...
            if (hasPerformanceAnomaly || (symptom && /slow|perf|cpu|dtu|query|latenc/i.test(symptom))) {
                ...
                const serversIdx = idParts.findIndex((p) => p.toLowerCase() === "servers");
                const dbIdx = idParts.findIndex((p) => p.toLowerCase() === "databases");
                if (serversIdx !== -1 && dbIdx !== -1) {
                    ...
                    try {
                        ...
                        const fqdn = serverResult.resources[0]?.["properties_fullyQualifiedDomainName"];
                        if (fqdn)
                            serverFqdn = fqdn;
                    }
                    catch {
                    ...
                    const qsResult = await querySqlQueryStore(serverFqdn, dbName, timeframeHours);
                    if (qsResult.error) {

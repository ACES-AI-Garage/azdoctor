...
// ─── az CLI helper ──────────────────────────────────────────────────
function executeAzCommand(command, timeoutMs = 60000) {
    try {
...
function parseServerNameFromId(resourceId) {
    ...
    const serversIdx = parts.findIndex((p) => p.toLowerCase() === "servers");
...
function executeRestart(resourceType, resourceName, resourceGroup, subscription) {
    ...
    if (normalizedType === "microsoft.web/sites") {
        const result = executeAzCommand(`az webapp restart --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription}`);
        return {
            ...result,
    ...
    if (normalizedType === "microsoft.cache/redis") {
        const result = executeAzCommand(`az redis force-reboot --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --reboot-type AllNodes`);
        return {
            ...result,
...
function executeScaleUp(resourceType, resourceName, resourceGroup, subscription, scaleTarget, resourceId) {
    ...
    if (normalizedType === "microsoft.web/serverfarms") {
        ...
        const currentSkuResult = executeAzCommand(`az appservice plan show --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --query sku.name -o tsv`);
        const previousSku = currentSkuResult.success
            ? currentSkuResult.output
            : "unknown";
        const result = executeAzCommand(`az appservice plan update --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --sku ${scaleTarget}`);
        return {
            ...result,
    ...
    if (normalizedType === "microsoft.sql/servers/databases") {
        ...
        const currentObjResult = executeAzCommand(`az sql db show --name ${resourceName} --resource-group ${resourceGroup} --server ${serverName} --subscription ${subscription} --query currentServiceObjectiveName -o tsv`);
        const previousObjective = currentObjResult.success
            ? currentObjResult.output
            : "unknown";
        const result = executeAzCommand(`az sql db update --name ${resourceName} --resource-group ${resourceGroup} --server ${serverName} --subscription ${subscription} --service-objective ${scaleTarget}`, 120000);
        return {
            ...result,
    ...
    if (normalizedType === "microsoft.cache/redis") {
        const currentSkuResult = executeAzCommand(`az redis show --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --query sku.name -o tsv`);
        const previousSku = currentSkuResult.success
            ? currentSkuResult.output
            : "unknown";
        const result = executeAzCommand(`az redis update --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --sku ${scaleTarget}`, 120000);
        return {
            ...result,
...
function executeScaleOut(resourceType, resourceName, resourceGroup, subscription, scaleTarget) {
    ...
    if (normalizedType === "microsoft.web/serverfarms") {
        ...
        const currentCountResult = executeAzCommand(`az appservice plan show --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --query sku.capacity -o tsv`);
        const previousCount = currentCountResult.success
            ? currentCountResult.output
            : "unknown";
        const result = executeAzCommand(`az appservice plan update --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --number-of-workers ${scaleTarget}`);
        return {
            ...result,
...
function executeFailover(resourceType, resourceName, resourceGroup, subscription, resourceId) {
    ...
    if (normalizedType === "microsoft.sql/servers/databases") {
        ...
        const result = executeAzCommand(`az sql db replica set-partner --name ${resourceName} --resource-group ${resourceGroup} --server ${serverName} --subscription ${subscription} --failover`, 120000);
        return {
            ...result,
    ...
    if (normalizedType === "microsoft.documentdb/databaseaccounts") {
        const result = executeAzCommand(`az cosmosdb failover-priority-change --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --failover-policies`, 120000);
        return {
            ...result,
...
function executeFlushCache(resourceType, resourceName, resourceGroup, subscription) {
    const normalizedType = resourceType.toLowerCase();
    if (normalizedType === "microsoft.cache/redis") {
        const result = executeAzCommand(`az redis flush --name ${resourceName} --resource-group ${resourceGroup} --subscription ${subscription} --yes`);
        return {
            ...result,
...
// ─── Tool registration ──────────────────────────────────────────────
export function registerRemediate(server) {
    server.tool("azdoctor_remediate", "Execute safe, reversible remediation actions on Azure resources. Supports restart, scale, failover, and cache flush operations with risk ratings and dry-run mode.", {
        resource: z
            .string()
        ...
        dryRun: z
            .boolean()
            .default(true)
            .describe("When true (default), shows what would happen without executing. Set to false to actually execute."),
        scaleTarget: z
            .string()
            .optional()
            .describe("Target SKU/tier for scale_up, or instance count for scale_out (e.g., 'P1v3' or '3')"),
    }, async ({ resource, subscription: subParam, resourceGroup, action, dryRun, scaleTarget, }) => {
                    ...
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({
                                error: `Could not resolve resource '${resource}'. Ensure the resource exists and you have Reader access.${resolved.error
        ...
        if (action === "list_actions") {
            const applicable = AVAILABLE_ACTIONS.filter((a) => a.applicableTo.includes(normalizedType));
            const response = {
                resource: resourceName,
                resourceType,
                availableActions: applicable.length > 0
                    ? applicable
                    : [],
                message: applicable.length > 0
                    ...
                    : `No remediation actions are currently available for resource type '${resourceType}'. Supported types: ${[
                        ...new Set(AVAILABLE_ACTIONS.flatMap((a) => a.applicableTo)),
                    ].join(", ")}`,
            };
            return {
                content: [
        ...
        if (!actionDef.applicableTo.includes(normalizedType)) {
            const applicableActions = AVAILABLE_ACTIONS.filter((a) => a.applicableTo.includes(normalizedType));
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({
                            ...
                            availableActions: applicableActions.length > 0
        ...
        if ((action === "scale_up" || action === "scale_out") &&
            !scaleTarget) {
            return {
                content: [
                    {
        ...
        switch (action) {
            case "restart":
                execResult = executeRestart(resourceType, resourceName, resolvedResourceGroup, resolvedSubscription);
                break;
            case "scale_up":
                execResult = executeScaleUp(resourceType, resourceName, resolvedResourceGroup, resolvedSubscription, scaleTarget, resourceId);
                break;
            case "scale_out":
                execResult = executeScaleOut(resourceType, resourceName, resolvedResourceGroup, resolvedSubscription, scaleTarget);
                break;
            case "failover":
                execResult = executeFailover(resourceType, resourceName, resolvedResourceGroup, resolvedSubscription, resourceId);
                break;
            case "flush_cache":
                execResult = executeFlushCache(resourceType, resourceName, resolvedResourceGroup, resolvedSubscription);
                break;
            default:
                execResult = {
                    success: false,
        ...
        const response = {
            ...
            result: execResult.success
                ? `Successfully executed '${action}' on ${resourceName}.${execResult.output ? ` Output: ${execResult.output}` : ""}`
                : undefined,

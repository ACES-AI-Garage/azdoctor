...
import { correlateTimelines, clusterEvents, detectMetricAnomalies, detectChangeVelocity, } from "./correlator.js";
// ---------------------------------------------------------------------------
// Helpers to build realistic Azure-like events
// ---------------------------------------------------------------------------
function makeChange(time, event, opts = {}) {
    return {
        time,
        event,
        source: "ActivityLog",
...
function makeAnomaly(time, event, opts = {}) {
    return {
        time,
        event,
...
describe("detectChangeVelocity", () => {
    ...
    it("detects multiple separate bursts", () => {
        ...
        const burst1 = Array.from({ length: 5 }, (_, i) => makeChange(`2026-03-10T10:${String(i * 5).padStart(2, "0")}:00Z`, `Burst1 change ${i}`, { actor: "alice@contoso.com" }));
        // Second burst: 5 changes within 30 min, 3 hours later
        const burst2 = Array.from({ length: 5 }, (_, i) => makeChange(`2026-03-10T13:${String(i * 5).padStart(2, "0")}:00Z`, `Burst2 change ${i}`, { actor: "bob@contoso.com" }));

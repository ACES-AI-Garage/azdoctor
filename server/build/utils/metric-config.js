...
function loadThresholdOverrides() {
    ...
    if (globalWarn)
        overrides.globalWarning = parseInt(globalWarn, 10);
    const globalCrit = process.env.AZDOCTOR_THRESHOLD_CRITICAL;
    if (globalCrit)
        overrides.globalCritical = parseInt(globalCrit, 10);
    // Per-type overrides
 ...
 */
export function getMetricConfig(resourceType) {
 ...
 */
export function getDependencyQueries(resourceType, resourceGroup) {
...
//# sourceMappingURL=metric-config.js.map